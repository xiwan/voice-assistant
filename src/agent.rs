...
/// Outcome of running one connection: reconnect (agent died / wedged) or stop.
enum Exit {
    Reconnect,
    Shutdown,
}
...
/// Sleep for `dur` while still honoring a Shutdown that arrives during downtime.
/// Returns true if we should stop entirely.
fn wait_or_shutdown(cmd_rx: &Receiver<AgentCmd>, dur: Duration) -> bool {
    match cmd_rx.recv_timeout(dur) {
        Ok(AgentCmd::Shutdown) => true,
        Err(RecvTimeoutError::Disconnected) => true, // handle dropped
        _ => false, // timeout, or a command we drop while down — proceed to retry
    }
}
